"""
Step 5: Train YOLO models on combined dataset (internet_scraped + singapore_river).

Trains directly on gold/ (merged by 1_merge_gold.py).
...
# ----------------------------
# PHASE 2: Combined dataset — uses winner config from Phase 1
# Placeholder: update model/imgsz after Phase 1 completes
# ----------------------------
if __name__ == "__main__":
    experiments = [
        Experiment(run_name="combined_y11s_960",  model="yolo11s.pt", imgsz=960,  batch=24),
        Experiment(run_name="combined_y11m_960",  model="yolo11m.pt", imgsz=960,  batch=16),
        Experiment(run_name="combined_y11s_1280", model="yolo11s.pt", imgsz=1280, batch=16),
    ]
    rows = run_wave(experiments)

    winner = sorted(rows, key=lambda r: (r["ladder_recall"], r["ladder_precision"]), reverse=True)[0]
    print(f"\n{'='*60}")
    print(f"COMBINED WINNER: {winner['run_name']}")
